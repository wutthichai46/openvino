...
bool MlasGemmExecutor::isSupported(const FCConfig& key) {
    ...
    if (key.attrs.withBias) {
        const auto& biaDesc = key.descs.src[2];
...
MlasGemmExecutor::MlasGemmExecutor(const FCConfig& key, const MemoryArgs& memory, const ExecutorContext::CPtr context)
    : packedWeights(prepareWeightMemory(memory.at(ARG_WEI), context, !key.attrs.weightsNonTransposed)) {}

void MlasGemmExecutor::update(const MemoryDescArgs& descs, const MemoryArgs& memory) {
    const auto& weiDesc = descs.src[1];
    const auto& dstDesc = descs.dst[0];
...
void MlasGemmExecutor::execute(const MemoryArgs& memory) {
    const auto srcRawMemPtr = reinterpret_cast<float*>(memory.at(ARG_SRC)->getData());
    const auto weiRawMemPtr = static_cast<float*>(packedWeights->getData());
    const auto dstRawMemPtr = reinterpret_cast<float*>(memory.at(ARG_DST)->getData());
    const auto biasRawMemPtr = reinterpret_cast<float*>(memory.at(ARG_BIAS)->getData());
