...
bool DnnlPostOpsComposerNew::appendAttrPostOps(const FakeQuantizePostOp& postOp,
                                               bool isLastPostOp,
                                               bool doRounding,
                                               bool allowBinary) {
...
bool DnnlPostOpsComposerNew::appendLinear(const std::vector<float>& scale,
...
static MemoryPtr prepackDecompressionParams(const MemoryCPtr& params_ptr,
                                            bool needTranspose,
                                            const dnnl::engine& engine) {
...
void DnnlPostOpsComposerNew::appendDecompressionScales(const MemoryCPtr& scales_ptr, bool needTranspose) {
    if (scales_ptr == nullptr)
        return;

    auto scalesMem = prepackDecompressionParams(scales_ptr, needTranspose, engine);
...
void DnnlPostOpsComposerNew::appendDecompressionZeroPoints(const MemoryCPtr& zero_points_ptr, bool needTranspose) {
    if (zero_points_ptr == nullptr)
        return;

    auto zeroPointsMem = prepackDecompressionParams(zero_points_ptr, needTranspose, engine);
...
DnnlPrimitiveAttrs DnnlPostOpsComposerNew::compose() {
